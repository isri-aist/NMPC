...
using OptVariable = typename nmpc_fmpc::FmpcSolver<2, 1, 2>::Variable;

/** \brief FMPC problem for bipedal walking.

    State is [CoM_pos, CoM_vel]. Input is [ZMP].
    Running cost is CoM_vel^2 + (ZMP - ZMP_ref)^2.
    Terminal cost is (CoM_pos - ZMP_ref)^2 + CoM_vel^2.
 */
class FmpcProblemBipedal : public nmpc_fmpc::FmpcProblem<2, 1, 2>
{
public:
  struct CostWeight
  {
    ...
    double running_vel = 1e-12;
    double running_zmp = 1e3;
    double terminal_pos = 1.0;
    double terminal_vel = 1.0;
  };

public:
  FmpcProblemBipedal(double dt,
                     const std::function<double(double)> & ref_zmp_func,
                     const std::function<std::array<double, 2>(double)> & zmp_limits_func,
                     const std::function<double(double)> & omega2_func,
                     const CostWeight & cost_weight = CostWeight())
  : FmpcProblem(dt), ref_zmp_func_(ref_zmp_func), zmp_limits_func_(zmp_limits_func), omega2_func_(omega2_func),
  ...
  {
    const auto & zmp_limits = zmp_limits_func_(t);
    IneqDimVector g;
    g << -1 * u[0] + zmp_limits[0], u[0] - zmp_limits[1];
    return g;
  }
...
protected:
  ...
  std::function<std::array<double, 2>(double)> zmp_limits_func_;
...
TEST(TestFMPCBipedal, TestCase1)
{
  double dt = 0.01; // [sec]
  ...
  std::function<std::array<double, 2>(double)> zmp_limits_func = [&](double t) {
    // Add small values to avoid numerical instability at inequality bounds
    t += epsilon_t;
    return std::array<double, 2>{-0.2, 0.2};
  };
  ...
  auto fmpc_problem = std::make_shared<FmpcProblemBipedal>(dt, ref_zmp_func, zmp_limits_func, omega2_func);

  // Instantiate solver
  auto fmpc_solver = std::make_shared<nmpc_fmpc::FmpcSolver<2, 1, 2>>(fmpc_problem);
  fmpc_solver->config().horizon_steps = horizon_steps;

  // Initialize MPC
  double current_t = 0;
  FmpcProblemBipedal::StateDimVector current_x = FmpcProblemBipedal::StateDimVector(0, 0);
  OptVariable opt_variable(horizon_steps);
  for(auto & s : opt_variable.s_list)
  {
    s.setConstant(1.0);
  }
  for(auto & nu : opt_variable.nu_list)
  {
    nu.setConstant(1.0);
  }
  ...
  std::string file_path = "/tmp/TestFMPCBipedalResult.txt";
  std::ofstream ofs(file_path);
  ofs << "time com_pos com_vel planned_zmp ref_zmp omega^2 iter" << std::endl;
  while(current_t < end_t)
  {
    // Solve
    fmpc_solver->solve(current_t, current_x, opt_variable);
    if(first_iter)
    {
      first_iter = false;
      fmpc_solver->dumpTraceDataList("/tmp/TestFMPCBipedalTraceData.txt");
    }

    // Check ZMP
    FmpcProblemBipedal::InputDimVector current_u = fmpc_solver->variable().u_list[0];
    double planned_zmp = current_u[0];
    double ref_zmp = ref_zmp_func(current_t);
    EXPECT_LT(std::abs(planned_zmp - ref_zmp), 1e-2);

    // Dump
    ofs << current_t << " " << current_x.transpose() << " " << planned_zmp << " " << ref_zmp << " "
        << omega2_func(current_t) << " " << fmpc_solver->traceDataList().back().iter << std::endl;

    // Update to next step
    current_x = fmpc_problem->stateEq(current_t, current_x, current_u);
    current_t += dt;
    opt_variable = fmpc_solver->variable();
  }
